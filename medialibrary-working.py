...
class MainMenu:
    def __init__(self, window):
        ...
        filtermenu.add_command(label="Media Type", command=MediaFilter)
    ...
    def recordSort(self, method):
        ...
        final = sorted(dateindex, key=lambda x: x[0])
        ind = 0
        for record in final:
            index = final[ind][1]
            name = getName(index)
            ...
            date = getDate(index)
            type = getType(index)
            self.table.insert("", index, "", text=index, values=(name, author, date, type))
            #print(newline)
            ind += 1
...
class Filter:
    def __init__(self):
        ...
        self.title = tk.Label(self.top, text="", bg=bg)
        self.title.grid(column=2, row=1, padx=150, pady=100)
        self.filterEntry = tk.Entry(self.top)
        self.filterEntry.grid(column=2, row=2)
        self.filterbut = tk.Button(self.top, text="Filter", command=self.test, bg=button_colour, fg="white")
        self.filterbut.grid(column=3, row=2)
...
class MediaFilter(Filter):
    ...
    def filter(self):
        self.filter = self.filterEntry.get()
        with open("Database.txt", "r") as file:
            data = file.readlines()
        MainScreen.table.delete(*MainScreen.table.get_children())
        i = 0
        for lines in data:
            type = self.getType(i)
            if type == self.filter:
...
class AuthorFilter(Filter):
    def __init__(self):
        super().__init__()
        self.title.configure(text="Author Filter")
        self.filterbut.configure(command=self.filter)
